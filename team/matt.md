...
* こんにちは、マット・チャナです。私は応用・エージェント型AIエンジニアです。
* フロントエンドからバックエンドまでを含むエンドツーエンドの技術ソリューションの設計・開発を行っています。
* また、完全に機能し、可観測性を備えたインフラを構築するために必要な多数のマイクロサービスの設計・開発にも利用しています。
  * 可観測性, かかんそくせい, observability
  * 備える, そなえる, to be equipped with
  * 構築, こうちく, construction, building
* 最近は、エージェントメモリなどの実用的なアプリケーション機能を提供する応用AIエージェントのステートマシンの設計・実装に注力しています。
  * 実装, じっそう, implementation
  * 注力, ちゅうりょく, focus on
...
* また、エージェントを活用してOmiseaiを支えるデータを知的に生成・更新するマルチモーダルパイプラインにも大きな可能性があると思います。
  * 知的, ちてき, intelligent
  * 生成, せいせい, generation
* 私は、レストランとユーザーの双方にOmiseaiを愛してもらえるようなシステムを構築できると信じています。
